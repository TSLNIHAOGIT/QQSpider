...
class InitMessages(object):
    ...
    def __init__(self):
        ...
        self.bitVectorFlag = False; # True or False 设置是否使用BitVector进行判重，优先使用，如果报错，则改为False
        if(self.bitVectorFlag):
    ...
    def readQQ(self, file_dir):
        ...
        # add below for debug , does not need to fill in QQForSpider.txt
        print('qqlist',qqlist)
        if len(qqlist) == 0:
            qqlist.append('1330065671')
            # qqlist.append('2509072638')
            # qqlist.append('1029725413')#1029725413 lw
        return qqlist
