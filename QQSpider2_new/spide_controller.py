...
class SpideController(object):
    ...
    def __init__(self, my_messages=None):#my_messages是InitMessage类的一个实例
        ...
        self.db = self.mongo.QQ  # 打开MongoDB的QQ数据库self.db = self.mongo['QQ']
        print('self.db.name',self.db.name)
        print('self.db',self.db)
        self.bitVectorFlag = self.my_messages.bitVectorFlag
            ...
            if text_information:
                ...
                try:
                    for elem in spidermessage.newQQ:
                        if (self.bitVectorFlag):                        # 使用BitVector的方法
                            print("use BitVector check ");
                            # 使用BitVector的方法
                        ...
                        else:                                           # 没有使用BitVector的方法
                            print("not use BitVector check ");
                            if (int(elem) not in self.my_messages.qq_for_spide) and (   # 判断该QQ是否已经爬过
